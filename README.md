...
// HTMLSpanElement or other accordingly
...
**ExtendWithCustomProps**

Use a union type to extend the component with additional custom props
...
All custom props must be spread in the component signature, because they should not be passed to the DOM element

```tsx
export const ComponentName = ({ className, children, customProp, ...props }: ComponentNameProps) => {
  // Use custom prop without passing the DOM
  ...
};
```
...
Default prop values should be defined in the component signature using the spread props

```tsx
export const ComponentName = ({ className, children, customProp = "Default Value", ...props }: ComponentNameProps) => {
  ...
};
```
...
Some props must be merged to avoid overriding at the DOM element level, e.g., className, style, etc

```tsx
export const ComponentName = ({ className, style, children, ...props }: ComponentNameProps) => {
  const handleOnClick = () => {};
...
Event handlers should be merged using the composeEventHandler utility function
...
export const ComponentName = ({
  ...
  return (
    <div
      onClick={composeEventHandler(onClick, handleOnClick)}
      {...props}
    >
      {children}
    </div>
...
**ExternalUsageExample**

```tsx
const Container = () => {
  const onClick = (e: React.MouseEvent) => {
    /**
     * This will be called before the original implementation.
     * Use e.preventDefault() to prevent the original implementation from running.
     */
  };

  return <ComponentName onClick={onClick}></ComponentName>;
};
```

### ForwardRefPrimitive

The pattern follows the [DomPrimitive](#domprimitive)
...
```tsx
const Container = () => {
  // For reference type prefer to use exported component element type
  const componentRef = useRef<ComponentNameElement>();
...
**ControlledInputPrimitive**

For some DOM elements, like <textarea>, <input>, or others, use the following definition for the props

```tsx
export type ComponentNameProps = React.TextareaHTMLAttributes<HTMLTextareaElement>;

// or

export type ComponentNameProps = React.InputHTMLAttributes<HTMLInputElement>;
```

**CombineRefs**

In some cases, you should merge references for the DOM element to use it inside the component and forward it outside
...
export const ComponentName = React.forwardRef<ComponentNameElement, ComponentNameProps>(
  ({ children, ...props }, forwardedRef) => {
    ...
    return <input ref={useCombinedRef(inputRef, forwardedRef)} {...props} />;
  }
);
```
